        ...
        for epoch in range(self.start_epoch, config.max_epochs):
            for step, (example_image, example_lms, right_imgs, right_lmss, wrong_imgs, wrong_lmss) in enumerate(self.data_loader):
                ...
                if (step ) % (num_steps_per_epoch/50) == 0 :
                    fake_store = fake_im.data.permute(0,2,1,3,4).contiguous().view(config.batch_size*8,3,64,64)
                    torchvision.utils.save_image(fake_im.data,
                        "{}fake_{}.png".format(config.sample_dir,cc),normalize=True)
                    real_store = real_im.data.permute(0,2,1,3,4).contiguous().view(config.batch_size*8,3,64,64)
