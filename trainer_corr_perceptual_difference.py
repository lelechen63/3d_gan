            ...
            for step, (example, real_im, landmarks, right_audio, wrong_audio) in enumerate(self.data_loader):
                ...
                if (step + 1) % 1 == 0 or (step + 1) == num_steps_per_epoch:
                    ...
                    if not config.fake_corr:
                        print("[{}/{}][{}/{}] Loss_D: {:.4f}  Loss_G: {:.4f}, cosine_loss: {}, percetual_loss: {},  l1_loss: {}, ETA: {} second"
                            .format(epoch + 1, config.max_epochs,
                                    step + 1, num_steps_per_epoch,
                                    loss_disc.data[0], loss_gen.data[0], real_loss_cosine.data[0], loss_perceptual.data[0],0.2 * l1_loss_g.data[0] , eta))
                    else:
                        print("[{}/{}][{}/{}] Loss_D: {:.4f}  Loss_G: {:.4f}, cosine_loss: {}, fake_cosine_loss: {}, percetual_loss: {}, l1_loss: {}, ETA: {} second"
                            .format(epoch + 1, config.max_epochs,
                                    step + 1, num_steps_per_epoch,
                                    loss_disc.data[0], loss_gen.data[0], real_loss_cosine.data[0],
                                    fake_loss_cosine.data[0],  loss_perceptual.data[0],0.2 * l1_loss_g.data[0], eta))
                    log_value(
                    ...
                    log_value(
                        'percetual_loss', loss_perceptual.data[0], step + num_steps_per_epoch * epoch)
                    log_value(
                        'l1_loss', 0.2 *  l1_loss_g.data[0], step + num_steps_per_epoch * epoch)
