...
from dataset import LRWFarnebackDataset, VaganFarnebackDataset
...
class Trainer():
    def __init__(self, config):
        ...
        if config.dataset == 'grid':
            self.dataset = VaganFarnebackDataset(
                config.dataset_dir, train=config.is_train)
        elif config.dataset == 'lrw':
            self.dataset = LRWFarnebackDataset(
                config.dataset_dir, train=config.is_train)
    ...
    def fit(self):
        ...
        for epoch in range(self.start_epoch, config.max_epochs):
            for step, (example, real_im, landmarks, right_audio, wrong_audio, real_flows) in enumerate(self.data_loader):
                t1 = time.time()

                if config.cuda:
                    example = Variable(example).cuda(async=True)
                    real_im = Variable(real_im).cuda(async=True)
                    real_flows = Variable(real_flows).cuda(async=True)
                    ...
                    real_flows = Variable(real_flows)
                ...
                f_flows = self.flow_encoder(real_flows)

                avg_f_audio_deri = F.avg_pool1d(f_audio_deri, 16, 16).view(-1, 15)
                avg_f_flows = F.avg_pool1d(f_flows, 16, 16).view(-1, 15)

                loss_gen = self.bce_loss_fn(D_fake, self.ones)
                loss_cosine = self.cosine_loss_fn(avg_f_audio_deri, avg_f_flows, self.one_corr)
